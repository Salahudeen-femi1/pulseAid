  ...
  return (
    <Modal onClose={onCancel}>
      <div className="p-4 text-center">
        <div className="mb-4">
          <h4 className="text-xl text-dark-tetiary dark:text-light-tetiary font-semibold">
            {title}
          </h4>
          <p className="text-sm text-dark-tetiary dark:text-light-tetiary/50">
            {message}
          </p>
        </div>
        ...
        <div className="flex justify-center gap-2 mb-4">
          {Array.from({ length: 6 }).map((_, index) => (
            <input
              ...
              className="w-12 h-12 border border-primary/50 rounded-lg text-center text-xl font-semibold flex items-center justify-center outline-0 focus:border-primary text-dark-secondary dark:text-light-tetiary"
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-center items-center">
          <button
            onClick={onCancel}
            className="w-full md:w-1/2 h-12.5 rounded-lg border border-dark-secondary/50 dark:border-light-tetiary/20 text-gray-600 dark:text-light-tetiary/50 transition cursor-pointer disabled:cursor-not-allowed disabled:opacity-50"
            disabled={isLoading}
          >
            {cancelText}
          </button>
